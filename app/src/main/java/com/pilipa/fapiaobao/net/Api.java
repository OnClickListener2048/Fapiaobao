...
public class Api<T> {
     ...
     * @param baseViewCallback
     */
    public static void messageRead(String token, String id, final BaseViewCallback baseViewCallback) {
        OkGo.<NormalBean>get(String.format(MESSAGE_READ, id, token)).execute(new JsonCallBack<NormalBean>(NormalBean.class) {
            @Override
            public void onSuccess(Response<NormalBean> response) {
                    baseViewCallback.setData(response.body());
            }
